...
class TableSegmentor 
{
 ...
 private:
  ...
  bool serviceCallback(TabletSegmentation::Request &request, TableSegmentation::Response &response);
  ...
  void processCloud(const sensor_msgs::PointCloud2 &cloud,
                    TableSegmentation::Response &response, 
                    Table table);
 ...
 public:
  ...
  TabletSegmentor(ros::NodeHandle nh) : nh_(nh), priv_nh_("~")
  {
    ...
    segmentation_srv_ = nh_.advertiseService(nh_.resolveName("segmentation_srv"), 
                                             &TableSegmentor::serviceCallback, this);
 ...
 */
bool TableSegmentor::serviceCallback(TableSegmentation::Request &request, 
                                        TableSegmentation::Response &response)
{
  ros::Time start_time = ros::Time::now();
  std::string topic = nh_.resolveName("cloud_in");
  ROS_INFO("Table detection service called; waiting for a point_cloud2 on topic %s", topic.c_str());
  ...
  if (!recent_cloud)
  {
    ROS_ERROR("Tablet object detector: no point_cloud2 has been received");
...
void TableSegmentor::processCloud(const sensor_msgs::PointCloud2 &cloud,
...
int main(int argc, char **argv) 
{
  ...
  visual_perception::TableSegmentor node(nh);
